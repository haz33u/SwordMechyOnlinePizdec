--[[
	One-shot Studio Command Bar script.
	Creates fallback MobSpawns markers for Loc3–Loc7 under
	...
	3. Run.
	4. Drag markers to real art positions if needed.
	5. Save place.

	Each marker has attributes:
	  MobId      = e.g. "L5_CinderWolf"
	  Zone       = A/B/C/D/Boss
	  Model      = preferred model name (same as MobId by default)
...
local function ensureLocationsFolder()
...
for locationId = 3, 7 do
	...
	if spawnFolder and #spawnFolder:GetChildren() > 0 then
		print(string.format("[Markers] Loc%d already has %d markers — skipped", locationId, #spawnFolder:GetChildren()))
		continue
	end
