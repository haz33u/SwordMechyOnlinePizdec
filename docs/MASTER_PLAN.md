...
|----------|-----------------|-----|------------|
| **P0 VERIFY** | **Mobs spawning sunk into the ground** — third fix applied (`27b2771`). The first two attempts each fixed a real bug without stopping the sink, because the math *predicted* the pivot-to-feet distance. It is now a closed loop: place roughly, measure the actual lowest body point, shift by the leftover error — correct regardless of which cause was dominant. Also re-grounds after `model.Parent` is set, zeroes `HipHeight`, sets `IgnoreWater`, and rejects ray hits >12 studs from the marker. **Needs an in-game check**: Studio MCP was unreachable, so this is unverified. Each mob now carries `GroundY` / `GroundSource` / `GroundCorrection` attributes and warns past 6 studs — if it still sinks, read those numbers instead of guessing. Background in `docs/prompts/mob-grounding-diagnosis.md`. | Blocks playtest; mobs unclickable/unreadable | in-game playtest |
...
| P3 | **`PetModels.F` is not a pet.** Inspected 2026-08-03: slot `F` (Charon, `P1_50_R1`) is an imported asset pack — 290 separate models, 3003 parts, 586×139×512 studs (names like `BGS Bucket` → a Bubble Gum Simulator pet pack). The client clones the whole pack and crushes it into a 2-stud cube, so Charon currently looks like nothing and costs ~12× the part budget on its own. `PetModels.Charon` is a second identical copy of the same pack. Fix costs zero generations: the pack contains `Skull Reaper` (13 parts, grey neon, hooded skeletal — a ferryman) plus 289 other usable models. `tools/fix_pet_models.lua` does it under `DO_CHARON` — **left off deliberately: which model becomes Charon is a design call, not a code call.** Flip the flag or name a different child. | pick a model | PetModelConfig |
| P3 | **12 unused `PetModels` slots — 3297 parts replicating to every client for nothing.** Verified 2026-08-03 with `tools/compare_unused_slots.lua`: `Stiko`, `Charon`, `Morpheus`, `Torn`, `Nifel`, `Nightmare`, `Grommash` are byte-identical to the live slots those pets use; `Woodling`(1p), `Hekata`(1p), `Forestling`(6p), `Lurk`(6p) are older/simpler drafts of richer live slots (`A` 100p, `D` 36p, `C` 46p, `B` 59p); `S-A` (216p) is a stray Nereus no pet maps to. `tools/fix_pet_models.lua` moves them to `ServerStorage.RetiredPetModels` (does NOT delete) under `DO_RETIRE_UNUSED` — off by default. Worth ~6.5k parts off client replication together with the `F` fix. | confirm | — |
| P3 | Workspace root has 47 `ProceduralModel`s; 45 verified dups of `PetModels` slots (strong sig: part names + sizes + colors). **`StrayPetModelService` now deletes stray pet models from the Workspace root at boot** (root children only, skips Humanoids / `KeepInWorld` attribute / NPCs+Mobs folders), so players no longer see unowned pets standing on the map. `Nereid` is now installed into `PetModels.E-D`, so the `KEEP_NAMES` exemption for it has been removed and the Workspace copy is swept like the rest. | — | — |
...
9. **Equipped rarity rim** (`applyEquipRarityGlow`): square `SizeConstraint.RelativeYY` layers so the rim hugs the `ScaleType.Fit` art instead of the wider wrap box, `UICorner` in **scale** (`UDim.new(0.22, 0)`) so the corner stays round at any slot size, and every rarity gets a rim (higher tiers just brighter). Do not go back to fixed 12px corners or `1.008` overshoot — that read as an invisible square frame.
10. **Chrome hover** (`bindBtnHover`): close / presets / side tabs / equip-best / sell buttons get `centerPivot` + `UIScale` (1.05, press dip 0.94). Inactive side tabs also fade `ImageTransparency` 0.22 → 0.06. Scale and image transparency only — never tween `UIStroke.Thickness` (§10.12).
11. **Shop gamepass cards**: grid children keep `AnchorPoint (0,0)`; the centre pivot lives on an inner `Card` frame that carries `SlotScale`. A centred anchor on a `UIGridLayout` child shifted every card half a cell up-left and clipped it. Columns are adaptive (1 below 460px wide, else 2) and cell width comes from the container — no hard `200px` floor. `PriceChip` sits **above** the full-card `Hit` button so its own click and `AutoButtonColor` work; OWNED cards drop the hover scale.
