...
| P2 | **Pets → 100% unique.** Root cause of look-alikes: every pet maps to a *different* slot name, so no code check can catch it — the duplicates live in the CONTENT of differently-named slots (`Waifu`/`E-D`/`S-A` = one 216-part Nereus). Process now in `docs/PET_GENERATION_BRIEF.md` (4 axes: silhouette / rarity-locked palette / material / one signature detail; ≤250 parts). **Step 1: run `tools/audit_pet_models.lua`** in the Studio command bar → exact list of duplicate groups, missing models, over-budget models. **Step 2: `tools/install_pet_model_slot.lua`** installs the unique `Workspace.Nereid` (34p merfolk) into slot `E-D` for `P2_K_L2` — zero generations. Step 3: generate only what the audit flagged. Step 4: re-run audit; "to generate: 0" = pets done. Also rename Loc1–Loc2 slots (`A`, `2-F`, `E-D`, `Waifu`) to pet names like Loc3–7 already are, so a wrong model is visible in the Explorer. **Dead mapping `P2_K_M1` removed (2026-08-02).** | generation limit | PetModelConfig |
...
| P3 | Workspace root has 47 `ProceduralModel`s; 45 verified dups of `PetModels` slots (strong sig: part names + sizes + colors). **`StrayPetModelService` now deletes stray pet models from the Workspace root at boot** (root children only, skips Humanoids / `KeepInWorld` attribute / NPCs+Mobs folders), so players no longer see unowned pets standing on the map. `Nereid` still needed for the P2 fix — install it into `PetModels` before relying on the sweep. | Nereid fix first | — |
